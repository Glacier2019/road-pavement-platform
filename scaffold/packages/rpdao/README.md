...
> 实现：`packages/rpdao/`　验收：`tests/contract/test_dao_contract.py`
...
但骨架期没有 M3，M6（`services/api/app.py`）只能自己 `import psycopg`、自己持有
...
**数据出口**"）。M2（`services/ingest`）作为"接入/落库"这个动作本身，仍旧直接写库 ——
...
与 DDL v0.1 实测的 30 张基表**逐张对齐**（`test_dao_contract.py` 第 1、2 项断言）。

### 两张口径不要混

- **逻辑视图 25 表**：报告第五章的**对象域视图**，含尚未建物理表的实体
  （`geometry_point`、WE 时序、LO 的交通量 v85 与轮迹分布、RE 时序）。
- **物理 DDL 30 表**：已在 PostgreSQL 实测建的**基表**，含字典与治理支撑表。
...
1. **不允许在 DAO 之外出现 psycopg。** 由 `test_dao_contract.py` 第 4 项用 AST 断言
   `services/api/app.py` 里没有 psycopg import / ConnectionPool / `.execute(` / `cursor`。
