...
--
-- ✅ 这个检查**已经补上了**（原来这里是"待办，记在这里以免忘掉"）：
--   见 `tests/contract/test_ddl_migration_parity.py`（套件 `m1-装库一致`）。
--   它建一个一次性库，只跑 `10_ddl_v0.5.sql` 取一份快照，再依次施加 85..91
--   的全部 migration 取第二份快照，逐项比对表/列/约束/索引。
--
--   第一次跑就抓出**两个真问题**（都不是本文件，但都属于这一类）：
--     · 91_migrate_v05_side_vocab.sql 在全新装的库上**直接报错** ——
--       它的校验无条件要求 left/right 两侧都非空，而全新装的表一行数据都没有。
--     · `cross_section_ground_point.side` 的 check 约束在两条路径上**名字不同**
--       （DDL 里是行内写法 → PostgreSQL 自动命名 `..._side_check`；
--        91 里是显式命名 `ck_cross_section_ground_point_side`）。
--       已在 DDL 里改为显式命名，两条路径收敛。
-- ============================================================================
