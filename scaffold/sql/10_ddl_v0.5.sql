...
--                       94_migrate_v05_earthwork_transfer.sql、
--                       95_migrate_v05_earthwork_pits.sql（均幂等）
--                 ★★ K1 的来历（一条**被推翻的"不做"**）：
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- N 节：取土坑 / 弃土坑（.tsf 的「取土坑」「弃土坑」两张表）
--
-- ★★ 为什么单开 N 节、不并进 L/M 节：同一个 .tsf 文件里的**第三类东西**。
--    L1 earthwork_factor   = 设计**输入**（用户设的压实系数）
--    M1 earthwork_transfer = 设计**成果**（软件算的 27 次调配）
--    N1/N2 borrow_pit/spoil_pit = 设计**参数**（土源与弃土场）
--    三类性质不同，故分三节。（.tsf 是 1 文件 ↔ 多表，这已是第三次同源分节。）
--
-- ★★★ 本节的表**几乎全是出厂默认值** —— 这是最要紧的事实，实测：
--    取土坑写的是 松土/普通土/硬土/软石/次坚石/坚石 = 20/20/20/20/20/0（六项和恰为 100，
--    说明它是**比例**不是方量）。但：
--        `earthwork_transfer` 里 坑=0 的 **23 行**土石占比 **恒为 20/60/20**
--        `earthwork_transfer` 里 坑=1 的 ** 4 行**土石占比 **恒为 33.3/33.3/33.3**
--    两个都与 20/20/20/20/20/0 **不符** —— 所以这个比例**不是**喂进调配算法的设计参数，
--    只是纬地的出厂默认（用户没改过）。
--    ⚠ 我一开始按「取土坑的土石比例驱动了坑=1 的组成」去验算，**被实测否掉了**：
--      按 20/20/20/20/20/0 加权算 GCID 23 的 用土(压实) 得 52811.12，
--      而实测是 47874.2367；按「逐类 ÷ 系数」（18466.38/1.23 + /1.16 + /1.09）才得 47874.2 ✓。
--
-- ★★ 真值只有**桩号**：
--    取土坑.上路桩号 = 4100.000 —— 与 `earthwork_transfer` 里 `source_kind=1` 那 4 行的
--    取土点**完全一致**。`source_kind=1`（从取土坑取土）的语义就靠它落地。
--    弃土坑.上路桩号 = 1900 —— 而 `earthwork_transfer` 里**没有任何一行指向它**，完全没用上。
--
-- ★★ 占位符：土方总量 ≈1e11、石方总量 ≈1e10、总容量 ≈1e15 —— 都是**「无限」**，
--    不是容量。所以这三列用 **numeric(20,4)**（`numeric(18,4)` 只有 14 位整数，装不下 1e15）。
--    ⚠ 这会是**全库最宽的 numeric**（现最宽 numeric(16,6)）—— 这本身就是「它不是容量」的信号。
--
-- ★ 不新造 `is_default` 之类的标记列：语义写在 COMMENT 与数据字典里，
--   沿用 `design_file.file_kind_code IS NULL` 表示「纬地自己没给码」的既有做法。
--
-- 实测规模：各 1 行（本工程 section_id=6）。回滚 = DROP TABLE borrow_pit, spoil_pit;
-- ═══════════════════════════════════════════════════════════════════════════════
