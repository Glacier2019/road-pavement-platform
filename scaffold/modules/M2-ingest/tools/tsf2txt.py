...
EXPORT_TABLES: tuple[str, ...] = (
    ...
    "过程",         # 27 行 31 列 —— 调配过程，本工程**唯一有真账**的调配表（本版落地）
    "取土坑",       # 1 行 12 列 —— 土源。⚠ 几乎全是出厂默认值，真值只有桩号（本版落地）
    "弃土坑",       # 1 行  5 列 —— 弃土场。⚠ 同上，且本工程**完全没用上**（本版落地）
    "统计扩展",     # 334 行 73 列 —— 逐桩调运（调出/借入）。⚠ 大半列恒 0（本版落地）
    "土方调配扩展记录",  # 334 行 46 列 —— 逐桩填方来源（填/利/缺）。⚠ 同上（本版落地）
)
