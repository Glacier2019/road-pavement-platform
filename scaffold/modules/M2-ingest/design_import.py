...
#
#  ⚠ 只收**明确登记过的**，不做"见到就收"。理由：`.cys`（涵洞系统参数）是
#    **软件参数**不是工程数据，收了就把台账弄脏了（见 `_SYSTEM_PARAM_SUFFIX`）。
#    所以这张表必须一行一行加。
#  ★★★ 更正（2026-09-23）：这里原先写的是「`.cys` 也在磁盘上、**也不在〔文件名〕
#    段里**」—— **后半句是错的**。实测 `.PRJ` **第 75 行显式声明**了它：
#        75| 涵洞系统参数文件(*.cys) = .\毕设.cys
#    结论（不收）侥幸对了，**理由是编的** —— 而编的理由下次会被别人当真。
#    **这是本仓库第三次同一个形状的缺陷：没去读就下了结论。**
#    前两次：`.dq` 被误判成「文本/pending」、`.tsf` 被误判成「读不了/blocked」。
#    **教训：理由和结论要一起验。**
...
_LEDGER_EXTRA_SUFFIX = {
    ...
    ".tsf": "土石方调配文件(*.TSF)",
    # ⚠ 更正（2026-09-23）：这条上面原来写着「`.tsf` 实测同样『磁盘上有、.PRJ
    #   里没提』，**尚未决定**」—— **两处都过期了**。`.tsf` 已定「甲 = 加」并
    #   在上面的 `_LEDGER_EXTRA_SUFFIX` 里；而且 `.PRJ` 到底提没提，本轮也一并
    #   查了：`.cys` 那条「没提」是错的（见上面 `_LEDGER_EXTRA_SUFFIX` 前的更正）。
    #   ⚠ 所以「磁盘上有、`.PRJ` 没提」这个**类**目前**没有成员** ——
    #     留着机制（`_LEDGER_EXTRA_SUFFIX`）是因为它验的是「声明 ∪ 磁盘」这个口径，
    #     不是因为它现在有活儿干。**一个永远为空的分类，迟早会被当成"没漏东西"。**
}
...
_SYSTEM_PARAM_SUFFIX = {
    # ★ 2026-09-23 用户定「不收」。**结论没变，但依据是重新量过的** ——
    #   原来这里只有一句"软件系统参数"，且 README/注释里配的理由是错的
    #   （说 `.PRJ` 没提它，实测提了）。三条量出来的判据：
    #   ① **厂商自己的魔数就标了 `_SYS_`**：`.cys` = `HINTSOFT_HD_SYS_1026`，
    #      而 `.hda` = `HINTSOFT_HD_PRJ_1045`。同一家软件，一个 SYS 一个 PRJ
    #      —— 这是厂商给的判据，比任何推测都硬。
    #   ② **116 组「编号/类型/值」全是 0~500 的档位**：可解析为数字 116/116，
    #      去重后**只有 22 个不同值**（0/0.7/1/2/3/3.5/4/4.5/5/5.5/6/7/8/9/10/
    #      13/15/20/25/40/251/500），**没有一个 > 10000**。
    #      涵洞的桩号/孔径/位置在 `.hda` 里 —— `.cys` **一个桩号都没有**。
    #   ③ **内容是出图配置**：`实心箭头标注`、`纬地涵洞标注字体`、`FSDB`/`FSDB_E`、
    #      填充图案 `ANSI31`/`GRAVEL`/`HONEY`/`AR-CONC`、图框 `[TITLE]`/`1:[SCALE]`、
    #      图框角点坐标 `420.718942, 282.577692`。
    #      **没有一个是「这条路的数据」。**
    #   ★ 交叉印证：`.cys` 的编号有 7 个（2003/2000/1045/2005/2002/2001/2009）
    #     在 `.hda` 里也出现，连 `.hda` 魔数里的 `1045` 都在其中 → 这 116 个是
    #     纬地的**全局参数编号表**，`.hda` 只是**引用**其中几个。
    #     **`.cys` 是参数定义，`.hda` 是数据。定义不收，数据才收。**
    #   ⚠ **但 `.cys` 不能当 `.hda` 的说明书**：里面没有任何孔径/类型/位置，
    #     全是显示参数。`.hda` 卡在「字段含义无文档」，这条路**实测走不通**，
    #     别试第二次。
    #   ⚠ **这段文字会进 `skipped_files`（运行时可读）**，故只写「这个文件自己
    #     为什么被跳过」，**不提别的后缀** —— 提了会污染按文本判后缀的检查
    #     （实测踩过：写「与 .hda 成对」让 `.hda` 三个字进了跳过理由，
    #       `test_design_import.py` 里那条「.hda 不得出现在 skipped 里」当场变红）。
    ".cys": ("软件系统参数（116 组参数档位 + 标注样式／字体／图框／填充图案；"
             "首行 HINTSOFT_HD_SYS_*）"),
}
