...
2) **物理表名**逐个取自 scaffold/sql/10_ddl_v0.5.sql，
   已在本机 PostgreSQL 实测确认（当前口径见下面第 4 条）。
...
4) **51 张域内逻辑表 ＋ 11 张跨域支撑表 = 62**，与 ``EXPECTED_PHYSICAL_TABLES``
   及实库实测数三者吻合，可作为自检。历史演进（每一步都是"接了新的设计文件格式"
   带来的，不是拍脑袋加的）：
     v0.2 = 21 域内 ＋ 11 跨域 = 32
     v0.3 = 33 域内 ＋ 11 跨域 = 44   （契约变更工单 #2：桩号一等实体化 + GE 域完整化）
     v0.5 = 51 域内 ＋ 11 跨域 = 62   （.HDM / .tsf 等逐桩与土方成果接入）
   ★ 这三个数字**必须与 DDL、数据字典同时改**。曾经只改了一处，
     于是文档写 42/44/55 各有各的说法，只有实库是 62 ——
     自检见 ``tests/contract/test_ddl_dict_catalog.py`` 与
     ``tests/contract/test_dao_contract.py`` 第 1/7 组。
===================================================================
"""
