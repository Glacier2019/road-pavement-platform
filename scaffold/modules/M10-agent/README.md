...
| 模块 | M10 |
| 主责组 | D 组（二期） |
| 阶段 | P4/P5 —— **本阶段不排期，仅冻结接口形状** |
| 服务目录 | `modules/M10-agent/` |
| compose profile | `m10` |
| 宿主端口 | 8025（容器内统一 8000） |
...
**产出**
* `contracts/agent/action_ticket.v0.1.schema.json`
...
docker compose -f docker-compose.skeleton.yml --env-file .env --profile m10 up -d --build agent
docker exec rp-agent python3 -c "import urllib.request;print(urllib.request.urlopen('http://localhost:8000/healthz').read().decode())"
```
...
> 三条硬线不可越：**不得直连库**、**写入只经 M2**、**动作必须人工确认**。
> 本模块刻意不提供绕过它们的配置开关。
