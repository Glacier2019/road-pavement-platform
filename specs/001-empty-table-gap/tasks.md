...
- [X] T001 实测记录基线：记下当前缺口数，作为「接完一条链」的对照
  `curl -s http://localhost:8024/gw/v1/catalog/tables`
  产出：62 / 36 有数据 / 26 空 的实测快照
- [X] T002 [P] 确认 5 个土方段在实库确为 0 行（SC-002 的前提）
  若**不是** 0，说明数据已被导入过 ⇒ 停止，回来改 spec（前提变了，结论也要变）
- [X] T003 [P] 确认 5 个段在 `weidi/__init__.py` 的 `IMPLEMENTED` 里
...
- [X] T005 [P] M2 新增 `GET /v1/design/capabilities` (FR-005)
...
- [X] T006 [P] M3 `Dao.table_census()` 带出 `owner` 字段 (FR-003)
  文件：`scaffold/modules/M3-rpdao/rpdao/pool.py`
  真源：`catalog.TABLE_OWNER`（**不另抄一份**，FR-003）
- [X] T007 M2 的段能力必须在 M6 可见（经 HTTP，**禁止 import**）(FR-007)
...
- [X] T019 [P] [US2] 排序稳定性测试：同输入跑两次，次序必须**完全一致**
- [X] T020 [P] [US2] 三键优先级测试：改动模块数 → 源是否具备 → 域序
- [X] T021 [US2] **元测试**：把某个键的比较方向反过来 ⇒ 测试必须红
- [X] T022 [US2] 实现三键稳定排序（research.md R3）(FR-006)
  同三维相同以 `table` 字典序决胜 ⇒ 完全确定
  ⚠ **不许**加「少数手工调序」—— 那会让 SC-003 无法成立
- [X] T023 [US2] 在响应里暴露 `priority_keys`（复核者要靠它重算）(FR-006/SC-003)
- [X] T024 [US2] `action` 文案 ＋ **指出缺哪一次导入** (FR-004/006)
...
- [X] T025 [P] [US3] 测试：`source_present` 不得被缓存（新增源文件后须立刻可见）(FR-002)
- [X] T026 [P] [US3] 测试：删除数据后重新盘点，如实变回空表（FR-009）
  **不得**因「文档说接完了」而继续报有数据
- [X] T027 [US3] M9 `tables.html` 改版：空表区显示 空因 / 归属模块 / 建议动作
- [X] T028 [US3] 页面提示必须说「源已具备」，**不得**说「可以导入」
  （后者是会落空的承诺 —— 见契约的措辞约束）
- [X] T029 [US3] `unknown` 在页面上一眼可见，**不得**默认折叠或并入有数据（INV-4）
- [X] T030 [US3] 新增静态资源必须同步 `scaffold/modules/M9-console/Dockerfile` 的 COPY
...
- [X] T031 [P] 更新 `scaffold/tests/contract/test_console.py`：空因展示组 + Dockerfile COPY 钉子
  另补两个断言（分析报告 MEDIUM-3）：
  · **分区父表空仍报空** —— `wim_axle_record` 清空后必须报父表空，
    不得因 39 个分区存在而报有数据（回归钉子，防未来改坏）
  · **登记与实测不符要可见** —— 模块登记说"骨架"而该表其实已通时，
    以实测为准且不符处可见
- [X] T032 [P] 更新 `scaffold/modules/M9-console/README.md`（仍写着「前端界面 TODO / 骨架」）
- [X] T033 [P] 更新 `scaffold/sql/数据字典-v0.5.md` 中与缺口相关的表述（若有）
- [X] T034 跑完整 `quickstart.md` 七组验收，逐条留证据
...
- [X] T035 重建 M2/M6/M9 容器并实测（`DOCKER_BUILDKIT=0`，本沙箱 BuildKit 不可用）
- [X] T036 提交：提交信息写明「为什么」与「靠什么防住」

### 分析报告追加（`analysis.md` Remediation，本轮已并入正文）

- [X] T037 [US1] **元测试**：FR-010 服务端不得猜
